...
@Tag(name = "Gender", description = "REST API para gestionar el catálogo de géneros (MDM).")
public interface GenderApi extends GenericApi<GenderDto> {
    String NAME_API = "Gender";
    ...
    ResponseEntity<GenderDto> getGenderById(
    ...
    ResponseEntity<GenderDto> createGender(
            @Parameter(description = "Detalles del " + NAME_API
                    + " a crear.", required = true) @Valid @RequestBody @NonNull GenderDto dto);
    ...
    ResponseEntity<Void> deleteGender(
            @Parameter(description = "ID del " + NAME_API
    ...
    ResponseEntity<PageResponseDto<GenderDto>> getPaginatedGenders(
            @NonNull GenderSearchDto searchDto,

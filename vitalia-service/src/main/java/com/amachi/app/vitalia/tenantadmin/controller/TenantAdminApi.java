package com.amachi.app.vitalia.tenantadmin.controller;
...
@Tag(name = "TenantAdmin", description = "REST API para gestionar detalles del tenant admin: crear, actualizar, obtener y eliminar.")
public interface TenantAdminApi extends GenericApi<TenantAdminDto> {
    String NAME_API = "TenantAdmin";
    ...
    ResponseEntity<TenantAdminDto> getTenantAdminById(
    ...
    ResponseEntity<TenantAdminDto> createTenantAdmin(
            @Parameter(description = "Detalles del " + NAME_API + " a crear.", required = true)
            @Valid @RequestBody TenantAdminDto dto
    );
    ...
    ResponseEntity<TenantAdminDto> updateTenantAdmin(
    ...
    ResponseEntity<Void> deleteTenantAdmin(
    ...
    ResponseEntity<List<TenantAdminDto>> getAllTenantAdmins();
    ...
    ResponseEntity<PageResponseDto<TenantAdminDto>> getPaginatedTenantAdmins(TenantAdminSearchDto searchDto,
    ...
    @PatchMapping(value = ID + "/reset-password", produces = MediaType.APPLICATION_JSON_VALUE)
    ResponseEntity<Void> resetAdminPassword(@PathVariable Long id, @RequestParam(required = false) String newPassword);
}

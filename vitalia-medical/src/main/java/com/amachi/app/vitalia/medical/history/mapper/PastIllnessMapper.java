package com.amachi.app.vitalia.medical.history.mapper;

import com.amachi.app.core.common.mapper.AuditableIgnoreConfig;
import com.amachi.app.core.common.mapper.BaseMapperConfig;
import com.amachi.app.core.common.mapper.EntityDtoMapper;
import com.amachi.app.vitalia.medical.history.dto.PastIllnessDto;
import com.amachi.app.vitalia.medical.history.entity.PastIllness;
import org.mapstruct.*;

/**
 * Enterprise Mapper para la gestión de patologías históricas (SaaS Elite Tier).
 */
@Mapper(config = BaseMapperConfig.class, builder = @Builder(disableBuilder = true))
public interface PastIllnessMapper extends EntityDtoMapper<PastIllness, PastIllnessDto> {

    @Override
    @AuditableIgnoreConfig.IgnoreTenantAuditableFields
    @Mapping(target = "medicalHistory.id", source = "medicalHistoryId")
    @Mapping(target = "isDeleted", ignore = true)
    PastIllness toEntity(PastIllnessDto dto);

    @Override
    @Mapping(target = "medicalHistoryId", source = "medicalHistory.id")
    @Mapping(target = "externalId", source = "externalId")
    PastIllnessDto toDto(PastIllness entity);

    @Override
    @AuditableIgnoreConfig.IgnoreTenantAuditableFields
    @BeanMapping(nullValuePropertyMappingStrategy = NullValuePropertyMappingStrategy.IGNORE)
    @Mapping(target = "medicalHistory.id", source = "medicalHistoryId")
    @Mapping(target = "isDeleted", ignore = true)
    void updateEntityFromDto(PastIllnessDto dto, @MappingTarget PastIllness entity);
}

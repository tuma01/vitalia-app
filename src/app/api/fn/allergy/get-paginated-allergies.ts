...
import { AllergySearchDto } from '../../models/allergy-search-dto';
import { PageResponseDtoAllergy } from '../../models/page-response-dto-allergy';

export interface GetPaginatedAllergies$Params {
  searchDto: AllergySearchDto;
...
export function getPaginatedAllergies(http: HttpClient, rootUrl: string, params: GetPaginatedAllergies$Params, context?: HttpContext): Observable<StrictHttpResponse<PageResponseDtoAllergy>> {
  const rb = new RequestBuilder(rootUrl, getPaginatedAllergies.PATH, 'get');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<PageResponseDtoAllergy>;
    })
  );
}

getPaginatedAllergies.PATH = '/mdm/allergy';

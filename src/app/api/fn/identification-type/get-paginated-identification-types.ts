/* tslint:disable */
/* eslint-disable */
/* Code generated by ng-openapi-gen DO NOT EDIT. */

import { HttpClient, HttpContext, HttpResponse } from '@angular/common/http';
import { Observable } from 'rxjs';
import { filter, map } from 'rxjs/operators';
import { StrictHttpResponse } from '../../strict-http-response';
import { RequestBuilder } from '../../request-builder';

import { IdentificationTypeSearchDto } from '../../models/identification-type-search-dto';
import { PageResponseDtoIdentificationType } from '../../models/page-response-dto-identification-type';

export interface GetPaginatedIdentificationTypes$Params {
  searchDto: IdentificationTypeSearchDto;

/**
 * Índice de la página a recuperar.
 */
  pageIndex?: number;

/**
 * Tamaño de la página.
 */
  pageSize?: number;
}

export function getPaginatedIdentificationTypes(http: HttpClient, rootUrl: string, params: GetPaginatedIdentificationTypes$Params, context?: HttpContext): Observable<StrictHttpResponse<PageResponseDtoIdentificationType>> {
  const rb = new RequestBuilder(rootUrl, getPaginatedIdentificationTypes.PATH, 'get');
  if (params) {
    rb.query('searchDto', params.searchDto, {});
    rb.query('pageIndex', params.pageIndex, {});
    rb.query('pageSize', params.pageSize, {});
  }

  return http.request(
    rb.build({ responseType: 'json', accept: 'application/json', context })
  ).pipe(
    filter((r: any): r is HttpResponse<any> => r instanceof HttpResponse),
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<PageResponseDtoIdentificationType>;
    })
  );
}

getPaginatedIdentificationTypes.PATH = '/mdm/identification-type';
